...
TEST_F(RoutePlannerTest, TestNextNeighbors) {
    route_planner.AddNeighbors(start_node);

    // Correct h and g values for the neighbors of start_node.
    std::vector<float> start_neighbor_g_vals{0.051776856, 0.082997195, 0.055291083, 0.10671431,};    
    std::vector<float> start_neighbor_h_vals{1.0858033, 1.0998145, 1.1831238, 1.1828455};

    for(int index = 0; index < start_neighbor_g_vals.size(); index++) {
        auto current = route_planner.NextNode();
        EXPECT_PRED2(NodesSame, current->parent, start_node);
        EXPECT_FLOAT_EQ(current->g_value, start_neighbor_g_vals[index]);
        EXPECT_FLOAT_EQ(current->h_value, start_neighbor_h_vals[index]);
        EXPECT_EQ(current->visited, true);  
    }
